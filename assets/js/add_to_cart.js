document.addEventListener("DOMContentLoaded", function () {
  ...
  if (decreaseBtn && increaseBtn && qty) {
    ...
    increaseBtn.onclick = () => qty.value = Math.min(99, parseInt(qty.value) + 1);
  }
  ...
  window.populateCartModal = (name, image, price, sizes, selected) => {
    ...
    document.getElementById("cartModalProductSizes").innerHTML = sizes
      .split(",")
      .map((s, i) => `<input type="radio" class="btn-check" name="cartModalProductSize" id="size${i}" value="${s.trim()}" ${s.trim() === selected ? "checked" : ""}><label class="btn btn-outline-primary" for="size${i}">${s.trim()}</label>`)
      .join("");
    qty.value = 1;
  };
  ...
  if (addToCartBtn) {
    addToCartBtn.addEventListener("click", function () {
      const size = document.querySelector('input[name="cartModalProductSize"]:checked');

      if (!size) {
        return showAlert("Select size!", "warning");
      }

      const productName = document.getElementById("cartModalProductName").textContent;
      const productImage = document.getElementById("cartModalProductImg").src;
      const productPrice = document.getElementById("cartModalProductPrice").textContent;
  ...
  modal.addEventListener("show.bs.modal", () => setTimeout(() => modal.classList.add("slide-in"), 10));
  modal.addEventListener("hide.bs.modal", () => modal.classList.remove("slide-in"));
});
